...
export async function GET() {
  try {
    ...
    const { stdout: logStdout } = await execAsync(
      `git -C ${getWorkspacePath()} log -n 5 --pretty=format:"%h|%s|%an|%ar"`,
      { encoding: 'utf8' }
    );
    ...
    const { stdout: branchStdout } = await execAsync(
      `git -C ${getWorkspacePath()} branch --show-current`,
      { encoding: 'utf8' }
    );
    let branch = branchStdout.trim();
    if (!branch) {
      const { stdout: shortHead } = await execAsync(
        `git -C ${getWorkspacePath()} rev-parse --short HEAD`,
        { encoding: 'utf8' }
      );
    ...
    const { stdout: revStdout } = await execAsync(
      `git -C ${getWorkspacePath()} rev-list origin/main..main --count`,
      { encoding: 'utf8' }
    );
    ...
    const { stdout: remoteStdout } = await execAsync(
      `git -C ${getWorkspacePath()} rev-parse origin/main`,
      { encoding: 'utf8' }
    );
  ...
  try {
    ...
    switch (action) {
      case 'stage':
        ...
        command = file ? `git -C ${getWorkspacePath()} add "${file}"` : `git -C ${getWorkspacePath()} add .`;
        break;
      case 'unstage':
        // 'restore --staged' is the modern way to remove files from the index
        // 'reset' is used for bulk unstaging
        command = file ? `git -C ${getWorkspacePath()} restore --staged "${file}"` : `git -C ${getWorkspacePath()} reset`;
        break;
      case 'commit':
        if (!message) {
        ...
        command = `git -C ${getWorkspacePath()} commit -m "${message.replace(/"/g, '\\"')}"`;
        break;
      case 'push':
        // Standard push to origin main
        command = `git -C ${getWorkspacePath()} push origin main`;
        break;
      default:
