...
import { Loader2, Edit3, Save, X, Search, ChevronRight, RefreshCw } from 'lucide-react';
...
export function FileViewerRight({
  ...
  const handleRefresh = async () => {
    if (!selectedFilePath || refreshing) return;
    setRefreshing(true);
    try {
      const url = `/api/files/content?file=${encodeURIComponent(selectedFilePath)}&path=${encodeURIComponent(selectedFileFolder)}`;
  ...
  const selectedFileSize = fileContent ? new Blob([fileContent]).size : 0;
  const selectedFileSizeKB = (selectedFileSize / 1024).toFixed(3) + ' KB';
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="p-6 border-b border-[#1F1F1F] bg-[#0D0D0D] flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {/* Refresh and Edit buttons when not already editing */}
          {!isEditing && (
            <>
              <button
                onClick={handleRefresh}
                disabled={refreshing}
                className="p-1.5 rounded-md hover:bg-[#1F1F1F] text-[#B8860B] hover:text-[#FFBF00] transition-all disabled:opacity-50"
                title="Refresh File"
              >
